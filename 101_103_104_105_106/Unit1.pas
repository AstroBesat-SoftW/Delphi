...
//bubble sort
var
dizi:array of integer;//dizi deðiþken tanýmladý
i,j,deger,adet:integer;
begin
  adet:=listbox1.Items.Count;//eleman sayýsýný al
  setlength(dizi,adet);//dizinin eleman sayýsýný belirle
  for i:=0 to adet-1 do
       dizi[i]:=strtoint(listbox1.Items.Strings[i]);//dizi aktar list box elemanlarý diziye aktar
...
begin
  adet:=listbox1.Items.Count;//eleman sayýsýný al
  setlength(dizi,adet);//dizinin eleman sayýsýný belirle
  orta_eleman:=adet div 2;//ortadaki elemana en yakýn olaný
  for i:=0 to adet-1 do
  dizi[i]:=strtoint(listbox1.Items.Strings[i]);//dizi aktar list box elemanlarý diziye aktar
...
begin
  ...
  for i:=0 to adet-2 do
   begin
    for j:=i+1 to adet-1 do //adet -2 yapmayý unutmayýn
      begin
...
var
  dizi:array of integer;//global dizi deðiþken
  adet:integer;
procedure sirala(alt,ust:integer);
  ...
  begin
    i:=alt;j:=ust;
    x:=dizi[(alt+ust) div 2]; // orta elemaný bul
    repeat
      while dizi[i]<x do inc(i);// i yi artýr
      ...
      if i<=j then
        ...
        until i>j;
          if j>alt then
          sirala(alt,j);//tekrar iþlet
          if i<ust then
          sirala(i,ust);//tekrar iþlet
  end;
...
begin
  ...
  setlength(dizi,adet);//dizi boyutlandýrýlýyor.
